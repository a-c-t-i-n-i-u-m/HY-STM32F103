...
extern void TFT_DrawLine(u8 x1, u16 y1, u8 x2, u16 y2);//画线
extern void TFT_DrawPoint(u8 x,u16 y);	   //画点
...
extern void drawbigpoint(u8 x,u16 y);	//画一个大点
...
//	while(1);
  ...
  /* Configure Key Button EXTI Line to generate an interrupt on falling edge */  
	...
	heyaodz_TPad();	//触摸屏校准
	M25P80_buf_ToRam_TP();	 //读取保存的值
	heyaodz_TPad0();
	heyaodz_JF();	//显示字符串
	TFT_ShowString(80,110,"Init....OK"); 
	TFT_ShowString(80,130,"Please Wait...OK"); 	
	while (1)                   //等待中断到来
  	{    						   
...
void heyaodz_TPad(void)	//触摸屏校准
{
	...
	TFT_CLEAR(0,0,240,320);     //清屏		
	POINT_COLOR=RED;
	TFT_ShowString(80,40,"Touch First Point");
	heyaodz_JF();	//显示字符串
	drawbigpointxx(20,20);	// 画点1

	while (1)                   //等待中断到来
  	{    						   
		
//	    if(!GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_6))_it1=1;
		if(_it1==1)
		{
			delay_ms(340);
			a=PEN;
...
void M25P16_RamTo_buf_TP(void) //保存校准数据  
{
	   ...
	   AT45_buffer[0]=	0X01; // 保存识别字
	  
	   AT45_buffer[1]=	Xs_1>>8; // 取高八位
	   AT45_buffer[2]=	Xs_1&0XFF; // 取低八位
	   AT45_buffer[3]=	Ys_1>>8; // 取高八位
	   AT45_buffer[4]=	Ys_1&0XFF; // 取低八位

	   AT45_buffer[5]=	Xs_2>>8; // 取高八位
	   AT45_buffer[6]=	Xs_2&0XFF; // 取低八位
	   AT45_buffer[7]=	Ys_2>>8; // 取高八位
	   AT45_buffer[8]=	Ys_2&0XFF; // 取低八位

	   AT45_buffer[9]=	Xs_3>>8; // 取高八位
	   AT45_buffer[10]=	Xs_3&0XFF; // 取低八位
	   AT45_buffer[11]=	Ys_3>>8; // 取高八位
	   AT45_buffer[12]=	Ys_3&0XFF; // 取低八位

	   AT45_buffer[13]=	Xs_4>>8; // 取高八位
	   AT45_buffer[14]=	Xs_4&0XFF; // 取低八位
	   AT45_buffer[15]=	Ys_4>>8; // 取高八位
	   AT45_buffer[16]=	Ys_4&0XFF; // 取低八位
       
	   M25P16_RamTo_buf(1,2,17); //将DF_buffer[]数组中指定长度的数据写入 	
}
...
void drawbigpoint(u8 x,u16 y)
{
	if(x>220&&y<12)
	{
    	...
    	TFT_ShowString(210,0,"CLR");//显示清屏区域
