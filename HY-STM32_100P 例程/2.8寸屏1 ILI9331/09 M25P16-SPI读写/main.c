...
uint8_t TxBuffer1[] ="SPI M25P80D-SU Example: This is SPI DEMO, 终端上出现这一行字，说明M25P16D的读写正常";
...
/* USART1 configuration ------------------------------------------------------*/
  ...
  */
  ...
  M25P16_RamTo_buf(1,40,TxBufferSize1); //将DF_buffer[]数组中指定长度的数据写入 
  M25P80_buf_ToRam(1,40,TxBufferSize1); //读取数据  
  ...
  if(a==1) {	
  	USART_OUT(USART1,&AT45_buffer[0],TxBufferSize1);	  
  } 
   while (1)
  {	
  }
